...
macro_rules! error_slot {
    ($name:ident) => {
        fn $name() -> &'static Mutex<Option<String>> {
            static SLOT: OnceLock<Mutex<Option<String>>> = OnceLock::new();
            SLOT.get_or_init(|| Mutex::new(None))
        }
    };
}
error_slot!(input_error_slot);
error_slot!(pack_error_slot);
error_slot!(audio_error_slot);
...
/// Snapshot merged into the `status` response.
