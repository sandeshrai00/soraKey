...
mod tests {
    ...
    fn a_null_soundpack_costs_only_that_field() {
        let document = config_json_with(&[
            ...
            ("enable_sound", serde_json::json!(false)),
        ]);
        ...
        assert!(!restored.enable_sound, "and so must the rest");
    }
    ...
    fn a_missing_field_defaults_without_losing_the_rest() {
        ...
        assert!(
            restored.selected_audio_device.is_none(),
            "the absent field takes its default"
        );
        assert_eq!(
            restored.volume, 0.8,
