...
fn main() {
    ...
    let _lock = match acquire_lock() {
        ...
        Err(e) => {
            eprintln!("sorakey: cannot create lock file: {e}");
            std::process::exit(1);
        }
    };
    ...
    let (keyboard_tx, keyboard_rx) = bounded::<String>(1024);
    let (hotkey_tx, hotkey_rx) = bounded::<String>(1024);
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // ourselves (unit + binary) instead of haunting the session. A menu
        // disable touches only shell.json (and the shell's teardown can miss
        // it), so the daemon also exits when the plugin is no longer enabled.
        crate::libs::orphan::check_at_startup();
        crate::libs::orphan::spawn_watchdog();
        crate::libs::orphan::spawn_disabled_watchdog();
    }
