...
pub(super) struct EngineState {
    ...
    /// the worker lands. Just the `soundpack_id`.
    pub(super) queued_load: Option<String>,
...
/// Opens the system default with a bounded retry (3 × 2s) — the old
/// forever loop blocked the select loop and queued keystrokes in RAM.
    ...
    for attempt in 1..=3 {
        match open_stream(device_manager, None) {
            ...
            Err(e) => {
                last_err = e.clone();
                crate::always_eprint!(
                    "❌ [AudioEngine] no audio device available (attempt {}/3): {} - retrying in 2s",
                    attempt,
                    e
                );
...
impl EngineState {
    fn new() -> Self {
        ...
        let (stream, stream_handle, opened_device_id, device_rate) =
            match config.selected_audio_device.as_deref() {
                Some(id) => match open_explicit_retrying(&device_manager, id) {
                    Ok((s, h, oid, rate)) => (Some(s), Some(h), oid, rate),
                    Err(e) => {
                        crate::always_eprint!("❌ [AudioEngine] degraded — no device: {}", e);
                        crate::state::status::set_audio_result(
                            false,
                            Some(format!("no audio device: {}", e)),
                        );
                ...
                None => match open_default_retrying(&device_manager) {
                    Ok((s, h, oid, rate)) => (Some(s), Some(h), oid, rate),
                    Err(e) => {
                        crate::always_eprint!("❌ [AudioEngine] degraded — no device: {}", e);
                        crate::state::status::set_audio_result(
    ...
    fn handle_key_event(&mut self, code: &str, down: bool) {
        ...
        if self.stream_handle.is_none() {
            // degraded: no device, drop key silently (no log per keystroke — see test)
            return;
        }
        ...
        play_segment(handle, segment, self.volume, &mut self.key_sinks);
    }
    ...
    fn switch_device(&mut self, device_id: Option<String>) -> Result<String, String> {
        ...
        // pack re-decode is handled off-thread in handle_command (see
        // P2-10) so a keystroke is never queued behind a device-switch decode.
...
fn handle_command(cmd_tx: &Sender<AudioCommand>, state: &mut EngineState, command: AudioCommand) {
    match command {
        ...
        AudioCommand::LoadKeyboardPack { soundpack_id } => {
            ...
            if state.loading_pack {
                state.queued_load = Some(soundpack_id);
                return;
            }
            state.loading_pack = true;
            spawn_load_worker(cmd_tx, seq, soundpack_id, state.device_rate);
        }
        AudioCommand::PackLoaded(seq, result) => {
            ...
            if let Some(queued_id) = state.queued_load.take() {
                if queued_id.is_empty() {
                    ...
                    state.loading_pack = true;
                    spawn_load_worker(cmd_tx, queued_seq, queued_id, state.device_rate);
                }
            }
        }
        AudioCommand::SwitchDevice(device_id) => {
            ...
            state.loading_pack = false;
            state.queued_load = None;
            let reprepare_id = state
                .pack
                .as_ref()
                .map(|p| p.soundpack.id.clone())
                .or_else(|| {
                    let cfg = crate::state::settings_saver::current();
                    if cfg.keyboard_soundpack.is_empty() {
                        None
                    } else {
                        Some(cfg.keyboard_soundpack)
                    }
                });
            match state.switch_device(device_id) {
                Ok(_) => {
                    crate::state::status::set_audio_result(true, None);
                    if let Some(id) = reprepare_id {
                        if let Some(rate) = state.device_rate {
                            state.pack = None;
                            state.key_sinks.clear();
                            state.load_seq += 1;
                            let seq = state.load_seq;
                            state.pending_pack_seq = Some(seq);
                            state.loading_pack = true;
                            spawn_load_worker(cmd_tx, seq, id, Some(rate));
                        }
                    }
                }
                Err(e) => {
