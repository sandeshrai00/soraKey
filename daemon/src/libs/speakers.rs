...
impl DeviceManager {
    ...
    pub fn get_output_devices(&self) -> Result<Vec<DeviceInfo>, String> {
        ...
        // Identical USB devices collide on name-hash IDs (same name → same
        // id). Keep the stable id (saved settings reference it) and first-match
        // semantics, but warn so the duplicate is visible in logs.
        {
            ...
            for d in &devices {
                if !seen.insert(d.id.clone()) {
                    *dups.entry(d.name.clone()).or_default() += 1;
                }
            }
            if !dups.is_empty() {
                let names: Vec<String> = dups
                    ...
                    .collect();
                crate::always_eprint!(
                    "⚠️ [DeviceManager] duplicate device id for name(s): {} — switching keeps first match",
                    names.join(", ")
                );
            }
        }
