...
#[derive(Debug, Clone)]
// `config_version` / `is_valid_v2` are only observed by tests, never by
// production code — without this the non-test build warns.
#[allow(dead_code)]
...
pub fn validate_soundpack_config(config_path: &str) -> SoundpackValidationResult {
    ...
    if let Some(version) = config_version.filter(|v| *v > 2) {
        ...
        // explicit V2
        validate_v2_structure(&config, config_version)
        ...
        validate_v2_structure(&config, None)
    } else {
        // unknown shape
...
fn validate_v2_structure(config: &Value, config_version: Option<u32>) -> SoundpackValidationResult {
    let mut missing_fields = Vec::new();
