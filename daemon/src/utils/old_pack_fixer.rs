...
pub fn convert_v1_to_v2(
    ...
    let v1_define_type = config
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("single");
    let definition_method = "single"; // V1 always converts to single (multi is concatenated)
...
pub fn back_up_existing_file(path: &Path) -> Result<Option<PathBuf>, String> {
    ...
    while target.exists() {
        ...
        if attempt > 2 {
            // ponytail: rotation ceiling — in a repeated-conversion loop the
            // backups are identical copies of the same unchanged file, so the
            // oldest (unnumbered base) is dropped and its slot re-taken
            // instead of erroring; at most 3 backup files exist. Upgrade to
            // newest-N shift if distinct file versions ever need to survive.
            if base
                .as_path()
                .symlink_metadata()
