...
pub fn decode_interleaved(path: &str) -> Result<(Vec<f32>, u16, u32), String> {
    ...
    // ~345MB). The header's `n_frames` is attacker-controlled — a huge value
    // must not drive a matching `reserve` — and a 100MB compressed file can
    // decode to multi-GB of PCM, so the decode loop below also hard-rejects
    // past the cap. A real keyboard click is milliseconds, not hours.
    ...
    loop {
        ...
        if let Some(b) = &mut buf {
            ...
            if samples.len() > MAX_SAMPLES {
                return Err(format!(
                    "Refusing to buffer {} samples from '{}' — decoded size cap exceeded (file too long or corrupt)",
                    samples.len(),
                    path
                ));
            }
