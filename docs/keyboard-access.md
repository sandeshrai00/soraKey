...
### Option A — uninstall keeps permission (by design)

Uninstall **keeps** the keyboard permission (udev rule + ACL survive — by design, so a reinstall re-asks nothing). To remove the permission itself:

```bash
sudo ~/.local/lib/sorakey/sora-keyboard-revoke.sh  # one password
```
...
# 2. Revoke live ACLs on current keyboard nodes (udev trigger revokes future nodes, setfacl clears current ones) — scoped to keyboards only (matches revoke script)
for d in /dev/input/event*; do [ -e "$d" ] || continue; udevadm info --query=property --name="$d" 2>/dev/null | grep -qx "ID_INPUT_KEYBOARD=1" && { pkexec bash -c 'setfacl -b "$1"' _ "$d" 2>/dev/null || sudo setfacl -b "$d" 2>/dev/null; }; done; udevadm trigger --subsystem-match=input --action=change 2>/dev/null || true
...
```bash
# revoke (as above) — setfacl scoped to keyboards only
pkexec bash -c 'rm -f /etc/udev/rules.d/70-sora-keyboard.rules /etc/udev/rules.d/70-sorakey-keyboard.rules && udevadm control --reload-rules && udevadm trigger --subsystem-match=input --action=change; for d in /dev/input/event*; do [ -e "$d" ] || continue; udevadm info --query=property --name="$d" | grep -qx "ID_INPUT_KEYBOARD=1" && setfacl -b "$d"; done 2>/dev/null'
...
No. The rule persists across reboots and `omarchy update` until you run `sudo ~/.local/lib/sorakey/sora-keyboard-revoke.sh`.
