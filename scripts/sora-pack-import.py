...
def get_audio_duration_ms(audio_path, zf=None, zip_prefix=None):
    ...
    # ffprobe for everything else (whitelist suffix — untrusted entry)
    tmp = None
    try:
        _raw_suffix = os.path.splitext(audio_path)[1].lower()
        if _raw_suffix not in (".wav", ".mp3", ".ogg", ".flac", ".m4a", ".aac", ".opus", ".wma"):
            _raw_suffix = ".bin"
        # cap length to 10 (whitelist already short, but hard cap)
        _raw_suffix = _raw_suffix[:10]
        with tempfile.NamedTemporaryFile(suffix=_raw_suffix, delete=False) as f:
            f.write(data)
            tmp = f.name
        try:
...
def find_config_in_zip(zf):
    ...
    for name in zf.namelist():
        if name.startswith("__MACOSX/") or "/__MACOSX/" in name:
            continue
        if name.endswith("/") or os.path.basename(name) != "config.json":
            continue
        depth = name.count("/")
...
def import_zip(zip_path):
    ...
    with zf:
        ...
        # An id colliding with a bundled pack (sugar65, ...) is destroyed on
        # the next shell start: the pack sync diffs the share-dir copy against
        # the bundled original, sees "different", and rm -rf's the user's
        # import — silently, and the restored bundled pack can't be deleted to
        # break the cycle. Reject before anything is touched.
        bundled_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "daemon", "soundpacks", "keyboard")
        if os.path.isdir(bundled_dir) and soundpack_id in os.listdir(bundled_dir):
            return None, f"Pack id '{soundpack_id}' collides with a bundled pack — rename it in config.json and re-import."
        ...
        # Stage outside the live scan dir (parent of keyboard/) so a daemon
        # scan never sees a half-written pack; prune stale .tmp leftovers.
        keyboard_dir = os.path.dirname(install_dir)
        parent_dir = os.path.dirname(keyboard_dir)
        os.makedirs(keyboard_dir, exist_ok=True)
        os.makedirs(parent_dir, exist_ok=True)
        for _d in (keyboard_dir, parent_dir):
            try:
                for _e in os.listdir(_d):
                    if _e.startswith(os.path.basename(install_dir) + ".tmp."):
                        _stale = os.path.join(_d, _e)
                ...
                pass
        tmp_dir = tempfile.mkdtemp(prefix=os.path.basename(install_dir) + ".tmp.", dir=parent_dir)
        skipped_traversal = []
        try:
            prefix = ""
            if strip_folder:
            ...
            if os.path.exists(install_dir):
                ...
                # Unique suffix avoids collision on same-second double-import.
                import time
                backup = install_dir + f".bak.{int(time.time())}.{os.getpid()}"
                if os.path.exists(backup):
                    backup = backup + f".{uuid.uuid4().hex[:6]}"
                    while os.path.exists(backup):
                        backup = install_dir + f".bak.{int(time.time())}.{os.getpid()}.{uuid.uuid4().hex[:6]}"
