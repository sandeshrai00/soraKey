...
# REPO auto-detect (handling B): dev checkout pulls from soraKey-Dev releases,
# public checkout pulls from soraKey releases. Falls back to public.
REPO="sandeshrai00/soraKey"
if _remote_url=$(git -C "$PLUGIN_DIR" config --get remote.origin.url 2>/dev/null); then
  _parsed=$(printf '%s' "$_remote_url" | sed -E 's|.*github\.com[:/]([^/]+/[^/.]+)(\.git)?$|\1|')
  if [[ "$_parsed" != "$_remote_url" && "$_parsed" == */* ]]; then
    REPO="$_parsed"
  fi
  unset _remote_url _parsed
fi
...
if [ "${FLOCKED:-}" != 1 ] && command -v flock >/dev/null 2>&1; then
  if ! env FLOCKED=1 flock -w 90 "$CACHE_DIR/build.lock" "$0" "$@"; then
    # flock passes the child's exit code straight through, so a real inner
    # failure looks identical to a 90s timeout. Probe the lock: still held
    # means we genuinely timed out waiting; free means the inner run failed
    # and already printed the real reason — don't mask it with the lock line.
    if ! flock -n "$CACHE_DIR/build.lock" true 2>/dev/null; then
...
sync_soundpacks() {
  ...
  trap 'rm -f "${stamp_tmp:-}"' RETURN EXIT
  local src id dst
...
arch="$(uname -m)"
case "$arch" in x86_64|aarch64) ;; *) echo "sora-build: no prebuilt for arch $arch" >&2; exit 1;; esac
asset="sorakey-${arch}"
...
if [[ -f "$LIB_DIR/source.sha256" ]] && [[ "$(cat "$LIB_DIR/source.sha256" 2>/dev/null)" == "$source_id" ]] && [[ -x "$BIN" ]] && [[ -f "$LIB_DIR/binary.sha256" ]] && [[ "$(sha256sum "$BIN" 2>/dev/null | cut -d' ' -f1)" == "$(cat "$LIB_DIR/binary.sha256" 2>/dev/null)" ]]; then
    ...
    if [[ -n "$expected" && "$expected" == "$actual" ]]; then
      if gh_can_verify; then
        ...
        if (( att_rc == 0 )); then
          ...
          sha256sum "$BIN" 2>/dev/null | cut -d' ' -f1 > "$LIB_DIR/binary.sha256" || true
          rm -rf "$tmp"
