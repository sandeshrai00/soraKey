...
#
# Files in ${XDG_RUNTIME_DIR}/sorakey/ (0700, fails if XDG_RUNTIME_DIR unset):
...
if [[ -z "${XDG_RUNTIME_DIR:-}" ]]; then
  echo "sora-terminal-grant: XDG_RUNTIME_DIR not set — cannot create secure result dir" >&2
  exit 1
fi
RUNDIR="$XDG_RUNTIME_DIR/sorakey"
if [[ -L "$RUNDIR" ]]; then
  echo "sora-terminal-grant: RUNDIR is a symlink: $RUNDIR" >&2
  exit 1
fi
mkdir -p "$RUNDIR"
