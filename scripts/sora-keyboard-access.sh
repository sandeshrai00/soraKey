...
# Loops over ALL keyboard nodes — a second seat may own the first.
find_keyboard_node() {
  local d
...
user_can_read_keyboard() {
  ...
  # no keyboard nodes at all -> fail; otherwise none readable
  if [[ $found_any -eq 1 ]]; then return 1; fi
  return 1
}
...
map_priv_error() {
  ...
  case "$low" in
    ...
    *privilege*tool*|*not*author*|*authoriz*|*agent*|*authority*|*session*|*polkit*|*display*|*terminal*required*) return 3 ;;
    *) return 1 ;;
  esac
}
...
if user_can_read_keyboard; then
  ...
  if [[ -f "$DST" ]] && ! cmp -s "$SRC" "$DST"; then
    note "Installed permission is outdated — run Enable keyboard sounds again to refresh."
    # try to refresh now while we have the chance (stale rule is never auto-refreshed otherwise)
    PK_ERR_FILE=$(mktemp)
    trap 'rm -f "$PK_ERR_FILE"' EXIT
    if run_privileged 2>/dev/null; then
      note "Permission refreshed."
    fi
    rm -f "$PK_ERR_FILE"
    trap - EXIT
  fi
...
# udev applies access shortly — check briefly as the user (20×1s for slow udev).
step "Checking"
for ((i = 0; i < 20; i++)); do
  if user_can_read_keyboard; then
