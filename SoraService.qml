...
Item {
  ...
  function notify(title, msg, urg) { Quickshell.execDetached(["/usr/bin/bash","-c",'omarchy-notification-wait 15 && exec omarchy-notification-send --app-name Sorakey -u "$1" "$2" "$3"', "_", urg || "low", title, msg]); if (title.indexOf("Import") !== -1 || title.indexOf("Soundpack") !== -1 || title.indexOf("Export") !== -1 || title.indexOf("Logs") !== -1) clearImportTimer.restart() }
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (!startProc.running && !root.stoppedFlag) {
        // start, never enable: an enabled unit resurrects the daemon at every
        // user-manager restart / login even when the plugin is disabled. The
        // shell recreates this service instance on every boot/re-enable, so
        // boot-time enable buys nothing but that resurrection path.
        startProc.command = ["/usr/bin/bash", "-c",
          'test -x "$HOME/.local/bin/sorakey" && test -f "$HOME/.config/systemd/user/sorakey.service" && systemctl --user start sorakey || exit 0']
  ...
  // ponytail: no onDestruction stop — every teardown (disable, remove,
  // uninstall, Stop button, reboot) is covered by the daemon's own
  // watchdogs + explicit stops; the old handler caused audio dropout on
  // every shell reload. See fixBug.md P3-5.
}
