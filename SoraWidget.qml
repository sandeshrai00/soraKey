...
Panel {
  ...
  Timer {
    ...
    interval: 5000
    repeat: true
    running: ctlProc.running || statusProc.running || packsProc.running || devicesProc.running
    onTriggered: {
      if (ctlProc.running) ctlProc.kill()
      if (statusProc.running) statusProc.kill()
      if (packsProc.running) packsProc.kill()
      if (devicesProc.running) devicesProc.kill()
    }
  }
  ...
  Timer {
    ...
    onTriggered: {
      ...
      grantProc.command = ["/usr/bin/bash", "-c",
        ...
        + "   if (( now - m < 8 )); then echo \"WAIT ALIVE\"; else echo GONE; fi;"
  ...
  function setHeroMatchTheme(on) {
    ...
    if (!logoWrite.running) {
      logoWrite.command = ["/usr/bin/bash", "-c", 'mkdir -p "$(dirname "$1")" && printf "%s" "$2" > "$1"', "_", root.logoModeFile, on ? "1" : "0"]
  ...
  function setRoundedCorners(on) {
    ...
    if (!roundedWrite.running) {
      roundedWrite.command = ["/usr/bin/bash", "-c", 'mkdir -p "$(dirname "$1")" && printf "%s" "$2" > "$1"', "_", root.roundedModeFile, on ? "1" : "0"]
  ...
  function remove() {
    ...
    uninstallProc.command = ["/usr/bin/bash", root.pluginDir + "/scripts/sora-uninstall.sh"]
  ...
  Process {
    ...
    onExited: function(exitCode) {
      if (exitCode === 0) {
        ...
        var ok = store.applyStatus(stdout.text)
        if (ok) {
          root.refreshAudioDevices()
          root.refreshPacks()
        } else if (store.audioError.indexOf("malformed") !== -1) {
          root.errorToast = "Daemon returned invalid status"
          clearErrorToast.restart()
        }
      } else {
  ...
  // runs sora-uninstall.sh in background (purge-only data wipe — the
      ...
      Column {
        ...
        Item {
          ...
          Column {
            ...
            Text {
              visible: !root.settingsOpen && root.errorToast !== ""
              width: parent.width
              text: root.shortText(root.errorToast)
              color: "#ff6b6b"
              opacity: 0.85
              font.family: root.bar.fontFamily
              font.pixelSize: Style.font.caption
...
SoraDropdown {
              ...
              // ponytail: overlay duplicates SoraDropdown inner MouseArea toggle + 300ms debounce; move into component when SoraDropdown owns lastClosedAt — keep both overlays until unified
              MouseArea {
                anchors.fill: parent
                hoverEnabled: false
            ...
            Row {
              ...
              Item {
                ...
                // ponytail: same overlay as barSectionDrop — keep until SoraDropdown owns toggle/debounce
                MouseArea {
                  anchors.fill: parent
                  hoverEnabled: false
                  cursorShape: Qt.PointingHandCursor
          ...
          Row {
            ...
            Text {
              ...
              RotationAnimation on rotation {
                from: 0; to: 360; duration: 900; loops: Animation.Infinite; running: checkingItem.visible
              }
            }
            Text {
              text: "Checking…"
              ...
              Column {
                ...
                Row {
                  ...
                  Text {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    transformOrigin: Item.Center
                    RotationAnimation on rotation { from: 0; to: 360; duration: 900; loops: Animation.Infinite; running: enablingCol.visible }
                  }
                  Text {
                    text: "Enabling…"
                    color: root.bar.foreground
